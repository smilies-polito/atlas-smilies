...
#: Where the embedding is stored, on every route. `atlas.pl` hands a basis name to scvelo
#: and scFates, both of which prepend ``"X_"`` themselves, so the embedding has to sit
#: here or plotting stops resolving it.
_EMBEDDING_KEY = "X_umap"

#: Key under which :func:`atlas.pp.wnn` and :func:`atlas.pp.knn` record how they built a
#: graph. Nothing else writes it, so it is what identifies a graph as one of ours.
...
def _graph_keys(mudata: MuData) -> list[str]:
    """Names of the graphs in ``mudata.uns`` that ATLAS built.

    Records written by other tools do not carry the provenance stamp, so they are passed
    over rather than mistaken for a graph.
    """
    found = []
...
def _detect_graph_key(mudata: MuData) -> str:
    """Find the one graph to embed when the caller named none.

    There is no default name to fall back on: the two routes store under different keys,
    so any single default would be silently wrong for the other, raising a missing-key
    error that reads as "no graph" to a caller who has one.
    """
...
def _resolve_route(mudata: MuData, key: str) -> tuple[dict, str]:
    """Read from the graph's own record how it was built.

    Both graph-building routes end at the same write-back step, so a graph this package
    produced always carries its route. One that does not was built elsewhere, and is
    ...
    # Route and parameters are written together, so they agree for anything this package
    # produced. A hand-assembled record can still disagree, and delegating one fails deep
...
def _graph_matrices(mudata: MuData, key: str) -> tuple[str, str]:
    """Names of the graph's matrices, resolved from the record describing it.

    No shape check is performed here: ``MuData`` refuses an ``obsp`` entry that is not
    square over the object's observations at assignment time, naming the key and both
    shapes, so a mismatched matrix cannot be present to be found.
    """
...
def umap(
    ...
    Embedding a second graph on the same object therefore replaces the first
    ...
    if route == _WNN_ROUTE:
        ...
        # muon prepares its own temporary object, reconstructing the combined
        # representation from the modalities the record names, so nothing is prepared here.
    ...
    # The matrix names are resolved from the record rather than assembled from the key, so
    # the temporary record must name what was actually resolved.
