...
from atlas.tl import spearman_correlation
...
def test_invalid_key1(mudata):
    with pytest.raises(KeyError, match=".obs"):
        spearman_correlation(mudata, key1="fake_key", key2="fake_key")


def test_invalid_key2(mudata):
    with pytest.raises(KeyError, match=".obs"):
        spearman_correlation(mudata, key1="pseudotime", key2="fake_key")


def test_nans(mudata):
    mudata.obs["entropy"] = np.nan
    with pytest.warns(UserWarning):
        stat, pval, ci = spearman_correlation(mudata=mudata, key1="pseudotime", key2="entropy")
...
def test_permutation_small_sample(mudata):
    rng = np.random.default_rng(SEED)
    ...
    with pytest.warns(UserWarning, match="sample size:"):
        stat, pval, ci = spearman_correlation(mdata_sub, key1="pseudotime", key2="entropy")
    assert stat > 0.8
    assert pval < 0.05
    assert ci is not None
    assert len(ci) == 2
...
def test_monotonic_not_linear(mudata):
    x = mudata.obs["pseudotime"].values
    y = np.exp(x)
    mudata.obs["entropy"] = y
    stat, _, _ = spearman_correlation(mudata, "pseudotime", "entropy")
    assert stat > 0.9
