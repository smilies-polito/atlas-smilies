...
# --------------------------------------------------------------------------------------
# the curves are the ones the superseded procedure produced
# --------------------------------------------------------------------------------------
...
def test_the_factor_is_the_same_curve_however_many_genes_are_fitted():
    """Its fit depends on pseudotime, its own expression and the lineage's weights — none of
    which vary by gene — so fitting it once for the whole call is the same fit."""
    mudata = _mudata()
...
def test_a_gene_is_the_same_curve_however_many_are_fitted():
    """Each gene is standardized from its own values, so genes never enter each other's fit."""
    mudata = _mudata()
...
# --------------------------------------------------------------------------------------
# lineages it cannot fit, and things it cannot find
# --------------------------------------------------------------------------------------


def test_a_lineage_with_negligible_weight_is_skipped_with_a_warning():
    """A real property of the inference, reported as it always was."""
    time = np.linspace(0, 1, N_CELLS)
...
# --------------------------------------------------------------------------------------
# the modalities the fit reads from
# --------------------------------------------------------------------------------------


@pytest.mark.parametrize("present", ["rna", "activity"])
def test_a_missing_modality_is_named(present):
    """Checked before anything is looked up, so the message names the modality rather than
    surfacing as a bare key error from the object. The class is public — `return_models` hands
    it to callers — so it guards itself rather than relying on the entry point that draws."""
    rng = np.random.default_rng(SEED)
    time = np.linspace(0, 1, N_CELLS)
