...
- `atlas.pl.trends` plots how a transcription factor's expression and the chromatin activity of the genes it may regulate move along pseudotime, per lineage. It supersedes `atlas.pl.plot_trends`. **What is computed is unchanged**. Several genes are drawn in one figure instead of one figure each. Lineage colours come from `.obs["terminal_states"]` and `.uns["terminal_states_colors"]`, falling back to the superseded mapping with a `FutureWarning`. `return_models` returns the fitted models instead of the axes.
