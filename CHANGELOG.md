...
- The platforms ATLAS supports are now recorded in `platform-support.toml` and checked rather than asserted. The record carries one entry per platform, Python version and installation shape, each stating the outcome expected of it, and the `Platforms` workflow resolves every one of them against PyPI and fails when reality differs. Installations are additionally performed and imported on Linux x86_64 and aarch64, macOS arm64 and x86_64, and Windows x86_64, and the test suite now runs on macOS and Windows as well as Linux.
